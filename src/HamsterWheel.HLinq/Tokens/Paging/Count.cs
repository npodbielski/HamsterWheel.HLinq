namespace HamsterWheel.HLinq.Tokens.Paging;

public sealed class Count(Range range) : TokenBase(range)
{
    public const string HLinqQueryToken = "count";

    public sealed class Possibility() : TokenPossibility<Count>(HLinqQueryToken)
    {
        protected override bool PreviousTokensMatch(List<IToken> previousTokens)
        {
            return previousTokens.Count == 0 || previousTokens is [.., RightSquareBracket, Dot];
        }

        protected override Count BuildImpl(Range range)
        {
            return new Count(range);
        }
    }
}