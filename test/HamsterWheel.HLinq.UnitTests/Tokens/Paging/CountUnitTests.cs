...
public class CountUnitTests
{
    ...
    public void Empty_WhenCalled_ReturnsTheSameInstance()
    {
        //arrange
        //act
        var actual = Count.Empty;

        //assert
        actual.Should().BeSameAs(Count.Empty);
    }
    ...
    public void Build_WhenCalled_ReturnsInstanceWithRange()
    {
        ...
        //act
        var actual = Count.Build(new(new(expected.Start.Value), new(expected.End.Value)));

        //assert
        var range = actual.Should().BeOfType<Count>().Which.Range;
